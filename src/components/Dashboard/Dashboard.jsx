...
const Dashboard = ({ reviews }) => {
  ...
  useEffect(() => {
    const fetchAllMovies = async () => {
      try {
        const allMovieData = await movieService.showAllMovies();
  ...
  const topMovies = allmovies
    .filter(
      (movie) => movie.average_rating !== undefined && movie.average_rating >= 1
    )
    .sort((a, b) => b.average_rating - a.average_rating)
    .slice(0, 5);

  const lowestMovies = allmovies
    .filter(
      (movie) => movie.average_rating !== undefined && movie.average_rating >= 1
    )
  ...
  const mostReviewedMovies = allmovies
    .map((movie) => {
      const reviewCount = reviewArray.filter(
        (review) => review.movie === movie.id
      ).length;
            ...
            {topMovies.length > 0 ? (
              topMovies.map((movie) => (
                <div key={movie.id} className="rated-card">
                  <div className="rated-average">
                    {movie.average_rating
                      ? movie.average_rating.toFixed(2) + " / 5.00"
                      : "N/A"}{" "}
                  </div>
